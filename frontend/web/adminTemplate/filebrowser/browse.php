...
###################################################################################
if(isset($_REQUEST['FCKeditor_Upload'])) {
    if (file_exists($IMAGES_BASE_DIR.$_FILES['FCKeditor_File']['name']) &&
        !empty($_FILES['FCKeditor_File']['name'])) {
        $fck_errMsg = "Error : File ".$_FILES['FCKeditor_File']['name']." exists, can't overwrite it...";
    } else {
        if (is_uploaded_file($_FILES['FCKeditor_File']['tmp_name'])) { //if file is uploaded
            $savefile = $IMAGES_BASE_DIR.$_FILES['FCKeditor_File']['name'];
            if (move_uploaded_file($_FILES['FCKeditor_File']['tmp_name'], $savefile)) { //move file from tmp to upload dir
                chmod($savefile, 0666); //change perms
                list($iWidth) = getimagesize($savefile);
                $fck_loadScript = "getImage('" . $_FILES['FCKeditor_File']['name'] . "', " . $iWidth . ");";
                $fck_sucMsg = "Your image ('" . $_FILES['FCKeditor_File']['name'] . "') was successfully uploaded.";
            }
